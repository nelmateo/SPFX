...
import * as strings from 'WpKnockoutWebPartStrings';
import WpKnockoutViewModel, { IWpKnockoutBindingContext } from './WpKnockoutViewModel';

let _instance: number = 0;

export interface IWpKnockoutWebPartProps {
  description: string;
}

export default class WpKnockoutWebPart extends BaseClientSideWebPart<IWpKnockoutWebPartProps> {
  private _id: number;
  ...
  protected onInit(): Promise<void> {
    ...
    const bindings: IWpKnockoutBindingContext = {
  ...
  private _registerComponent(tagName: string): void {
    ko.components.register(
      tagName,
      {
        viewModel: WpKnockoutViewModel,
        template: require('./WpKnockout.template.html'),
        synchronous: false
      }
    );
  }
